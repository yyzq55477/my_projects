...
Picasa.prototype = {
	...
	initData:function(data){
		...
		if (typeof(data) == 'undefined') {
			$(self.selector).each(function(index){
				...
				self.arrItems.push(	 Array(img,name,[0,0],''));
			});
		} else {
			for (var index in data) {
				$(this).attr('PicasaNo', index);
				if (data[index].type == 'Image') {
					var original = data[index].file;
					var thumb = data[index]['thumb-2'];
					var uuid = data[index]['uuid'];
					var node = data[index]['node'];
					var uuidExt = data[index]['uuidExt'];
					var file = data[index]['file'];
					img = [thumb,original];
					name = data[index]['filename'];
					self.arrItems.push(new Array(img,name,[0,0],'',uuid,node,uuidExt,file));
				}
			}
		}
	...
	// 初始化
	init : function(selectorUser,WindowUser,paintUrl) {
		this.selector = selectorUser;
		var self = this;	
		var selfBuild = false;	//是否根据传入的值搭建ifrem

		if(WindowUser !=null  && WindowUser instanceof Array){
			this.windowsStyleArray = WindowUser;
			selfBuild = true;
		}
		if(paintUrl != null ){
		...
		if(!this.inited) {
			...
			this.inited = true;
			if(selfBuild){
				$('#PicasaMask').css("left",WindowUser[0]+"px");
				$('#PicasaView').css("left",WindowUser[0]+"px");
			}
	...
	createFrame : function() {
		...
		this.frame.innerHTML =
			...
			'			<a href="javascript: void(0);" id="PV_Btn_Prev"><i class="icon-arrow-left"  style="margin-top: 5px;"></i></a>'+
			'			<a href="javascript: void(0);" id="PV_Btn_Slide"><i class="icon-play"  style="margin-top: 8px;"></i></a>'+
			'			<a href="javascript: void(0);" id="PV_Btn_Next"><i class="icon-arrow-right"  style="margin-top: 5px;"></i></a></div>'+
			'		<a href="javascript:void(0);" id="PV_rotate_Left" class="rotate"><i class="fa fa-rotate-left"></i></a>'+
			'		<a href="javascript:void(0);" id="PV_rotate_Right" class="rotate"><i class="fa fa-rotate-right"></i></a>'+
			'		<a href="javascript:void(0);" id="PV_Btn_Print"><i class="fa fa-print"></i></a>'+
			'	</div>'+
			'</div>';
	...
	createItems : function() {
		...
		for (var i=0; i<this.arrCount; i++){
			if(i>=200){
				...
				itemStr += '<li number="'+i+'" data-id= "'+ this.arrItems[i][4] 
				+'" data-node = "'+ this.arrItems[i][5] +'" data-ext = "'+ this.arrItems[i][6] +'" data-file = "'+ this.arrItems[i][7] +'">'
				+'<img src="'+ this.arrItems[i][0][0] +'" draggable="false"/></li>';
			}			
		}		
	...
	setFrameResize:function(){
		...
		self._setFrame();
		var winSize ;
		if(this.windowsStyleArray.length>0){
			winSize = this.windowsStyleArray;
		}else{
	...
	setElementEvent : function() {
		...
		//TODO 点击缩略图问题
		$('#PV_Btn_Print').off("click").bind('click',function(e){
			var id = $(self.selector+':eq('+self.currentNo+')').data('id');
			var images = {};
			images[id] = '<img src="' + getObject(id).file + '" />';
			printImages(images);
		});
		$('#PV_rotate_Left').off("click").bind('click',function(e) {
			self.rotate(false);
		});
		$('#PV_rotate_Right').off("click").bind('click',function(e){
			self.rotate(true);
		});
		$('#PV_Btn_Graffiti').off("click").bind('click',function(e){
			var id = $(self.selector+':eq('+self.currentNo+')').data('id');
			var obj = getObject(id);
			var path = filextApi.WEB_APP +'/filext-api?local=true&watermark='
					+ encodeURIComponent(obj.watermark) + '&node=' + obj.node
					+ '&uuidExt=' + obj.uuidExt + '&uuid=' + obj.uuid + '&method=putStorageContent'+'&replace=true'
					+'&cookie='+getCookie('J1DZYX_SESSION');
			...
			tz.setBasePainterHost(filextApi.config.config.paintUrl);
			tz.painter(painter , options);
			
			//涂鸦完成后的监听事件（保存成功隐藏并更新材料数量和刷新
			tz.onPainterMessage = function(message) {
				$('#painter').modal('hide');
				updateCateView();
				updateMaterialVersion();
				var id = $(self.selector+':eq('+self.currentNo+')').data('id');
				var obj = getObject(id);
				$("#PV_Picture").attr("src",obj.file);
			}
		});
		
		//涂鸦打开模态框
		function painter(url){
			var testT = setTimeout(function(){
				$('#iframePaint').attr('src',url)
			},500);
			//自适应高度宽度
			$('#painter').modal('show').css({
                width: $(window).width(),
	...
	rotate : function(clockwise) {
		var choose = $("body").find(".current").eq(0);
		var id = choose.data('id');
		var degree =  (clockwise?90:270);
		if(id != current){
			current = id;
			rotateDegree = degree;
		}else {
		...
		//var obj = getObject(id);
		//var degree = (obj.degree||0) + (clockwise?90:270);
		var node = choose.data('node');
		var ext = choose.data('ext');
		var file = choose.data('file');
		...
		if (res.success) {
			this.resetImage(file.replace(/(\.|-|\w)+\-(jpg|jpeg|gif|png|tif|bmp)/, res.data.fileName));			
		}
	},
	...
	// 加载图片
	loadImage : function() {
		$("#PV_Number").html((this.currentNo+1) + "/" +this.arrCount);
...
if(speed!=0) {
	...
	// 结束关闭
	close : function() {
		if (this.Error.style.display=='' || this.isIE()
