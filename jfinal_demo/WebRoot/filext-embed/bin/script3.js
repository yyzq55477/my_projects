﻿﻿var filextApi = null,
	...
	template = "<ul id='items' class='clearfix'>"+
			"{{#data}}<li class='item {{type}}Cls' id='item_{{uuid}}' data-id='{{uuid}}' title='{{title}}' data-href='{{var-0}}'>"+
	...
	thumbTemplate = "<table class='thumbTable'><tr><th style='width:30px'><input type='checkbox' id='checkAll'/></th><th width='40px'>序号</th>" +
				"<th>材料名称</th><th width='50px'>历史<br/>材料数</th>" +
				...
				"<td><span class='badge-version {{#if hasBaseFile}}base-material{{/if}}'><u>" + 
...
function initListView(){
	...
	if (filextApi.config.config.viewType == 'classical') {
		editId = $('li.item').eq(0).data('id');
		var activeItem = $('li.thumbItem.clearfix.active').parent();
		initDetailView();
	} else {
		var BulidWidow = [];
		var dragwrap = $("#dragWrap #items .ImageCls");
		if(dragwrap.length == 0) return;
		var left = $('#listView .guide').width();
		var domHegiht = $('#dragWrap').height();
		var domWidth = $('#dragWrap').width();
		BulidWidow[0] = left;
		BulidWidow[1] = domHegiht;
		BulidWidow[2] = domWidth;
		picasaGallery('#dragWrap #items .ImageCls',BulidWidow);
	}
...
function updateContent() {
	...
	$('.replace').off('click').click(function() {
		...
		filextApi.replace(replaceData, function(res){
	...
	$('.collection').off('click').click(function() {
		var activeItem = $('tr.thumbItem.active');
...
function updateMaterialVersion() {
	...
	var el = $('#listView tr.thumbItem.active');
...
function initListThumb() {
	if(privilege.enableGroup){
		...
		//如果第一个没有材料为空，就选中下一个，直到有材料为止，都为空还是选择第一个
		for(var i in initData){
			if(initData[i].children.length>0){
				if(i==0){
					break;
				}
				...
				thumbItems.eq(i).children('i').addClass('icon-white');
				currentId = thumbItems.eq(i).data('id');
				updateContent();
				break;
			}
		}
...
function showPages(isDetail) {
	...
	if (isDetail) {
		...
		$('#dragWrap ul').before('<span class="materialTitle">' + $('.thumbTable tr.active').data('title') + '</span>');
	}
...
function buildPdf() {
	...
	var uuids = [];
	var len = $('.thumbTable tr:gt(0) input:checked').length;
	var all = len==0 || $('.thumbTable tr:gt(0)').length==len;
...
//轮播组件
function picasaGallery(selector,WindowArr) {
	picasa.init(selector,WindowArr,filextApi.config.config.paintUrl);
	...
	picasa.changeImage($(selector).index($(selector)));
...
function printImages(images) {
	...
	for (var id in images) {
		// TODO 临时方式去掉水印
		LODOP.ADD_PRINT_IMAGE(0, 0, "100%", "100%", images[id].replace(/watermark=\w+/, 'watermark=%20'));
		LODOP.NewPage();
	}
