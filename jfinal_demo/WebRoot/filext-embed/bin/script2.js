﻿﻿var filextApi = null,
	...
	template = "<ul id='items' class='clearfix'>"+
			...
			// "<p class='operate'><span class='edit'
			// data-id='{{uuid}}'>编辑</span><i class='line'>|</i>"+
			"<p class='operate'>"+
...
function initListViewEvent(){
	...
	$('.highShoot').off('click').click(function(){
		...
		/*
		 * if (isIE()) { $('#modal-upload').modal('hide'); openHighShoot(); }
		 * else { showTip('高拍功能仅支持IE浏览器！'); }
		 */
		/*
		 * $('#upload-default').hide();
	...
	$('.highShootLt').off('click').click(function(){
		...
		if (isIE()) {
			...
			$('#modal-upload').css('display','hide');// modal('hide');
			openHighShootGoogle();
			// showTip('高拍功能仅支持IE浏览器！');
		}
	});
...
function updateContent() {
	...
	$('li.item').off('dblclick').dblclick(function() {
		...
		if (href) {
			if (href.slice(-30).indexOf('pdf')) {
// icv.viewBigPdf(href, true);
// var pdfHeight = doc.height() - 6;//iframe border=2
// doc.find('a.media').media({width : '100%', height : pdfHeight});
// doc.find('.view-big-container .media').css('margin-top', '0px');
				parent.icv.viewBigPdf(href, true);
...
function initListThumb() {
	 ...
	 * if(parent.icv.area!='zs'){ $('#numberOfMaterial').hide(); }
	 * preventBubble(e); var id = $(this).data('id'); var group =
	 * getGroup('uuid', id);
	 * 
	 * $('#groupTitle').html(group.description+" "+group.title); $('#modal-group
	 * .category-save-edit').data('code', group.code); $('#modal-group
	 * .category-save-edit').data('groupId', id); $('#modal-group
...
function buildPdf() {
	...
	// $('.thumbTable tr:gt(0) input' +
	// (all?'':':checked')).parent().parent().each(function() {
	$('.thumbTable tr:gt(0)').each(function() {
		uuids.push($(this).data('id'));
	});
	$('.thumbTable li:gt(0)').each(function() {
...
function uploadModalShow(){
	...
	}else if (uploadType == 'high-shoot-guoguang'){// 国光高拍 by yuanzp
		if (isIE()) {
