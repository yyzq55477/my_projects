﻿var filextApi = null,
	...
	template = "<ul id='items' class='clearfix'>"+
			...
			"<li id='native-file' class='collection'><img src='style/images/upload2.jpg' /><p class='title'>采集</p>"+
	...
	thumbTemplate = "<table class='thumbTable'><tr><th style='width:30px'><input type='checkbox' id='checkAll'/></th><th width='40px'>序号</th>" +
				...
				"</div></div>" +
				"<hr class='division'><div class='picList'><p class='text-danger'>身份证正反面</p><div class='btngroup'></div><div style='padding-top: 10px'></div></div>",
...
function initListViewEvent(){
	...
	/*九派 高拍 by gaofx 20180328*/
	$('.highShootJP').off('click').click(function(){
		if ($('#uploadType').is(':checked')) {
			uploadType = 'high-shoot-jp';
		}
		if (isIE()) {
			$('#modal-upload').modal('hide');
			openHighShootJP();
...
function uploadModalShow(){
	...
	}else if (uploadType == 'high-shoot-jp'){//方正高拍 by yuanzp
		if (isIE()) {
			openHighShootJP(); return ;
		} else {
			showTip('高拍功能仅支持IE浏览器！');
		}
	}
...
/* 打开国光高拍界面   by gaofx 20171108*/
function openHighShootJP() {
	window.showModalDialog('high-shoot-jp.html?v=16', window, "dialogHeight:600px;dialogWidth:1000px;");
}
