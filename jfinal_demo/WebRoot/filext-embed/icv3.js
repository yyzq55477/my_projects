...
(function() {
	...
	icv.render = function(id, bizCode, bizId, fuId, jgCode, params) {
		if(icv.config.websorket == "send"){
			...
			sendWebsorketMessage(sendJson);
			if(icv.config.renderIcv == false){
				return;
			}
		}
		...
		var cturl = this.config.appcontext + '/filext-embed/icv-ct3.html?v1.1';
	...
	icv.screen = function(webSocketUrl){
		loadScript(this.config.appcontext+"/filext-embed/tz.websocket.min.js",function(){
		    var websocket = new TzWebSocket(webSocketUrl, getScreenRequset);
		});
	}
	...
	function getScreenRequset(data){
		console.log(data);
		icv.setupConfig({
			appcontext : data.config.appcontext,// 业务系统上下文地址
			privilege : {}, 
			paintUrl : data.config.paintUrl,
		}).render('screenDiv', data.bizCode, data.bizId, data.fuId, data.jgCode, '');
	}
	function afterSend(data){
		console.log(data)
	}
	function sendWebsorketMessage(data){
		loadScript(data.config.appcontext+"/filext-embed/tz.websocket.min.js",function(){
			var webSocketUrl = data.config.websorketUrl;
			var websocket = new TzWebSocket(webSocketUrl,afterSend);
			websocket.setLevel(websocket.level.Master);
