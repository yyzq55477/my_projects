...
//			"<label><input class='checkpic' type='checkbox'data-id='{{uuid}}' /><span></span></label>"+
			...
			"<span class='download' data-id='{{uuid}}' data-lock='{{lockId}}'>下载</span><i class='line'>|</i>"+
			"<span class='print' data-id='{{uuid}}' data-lock='{{lockId}}'>打印</span><i class='line'>|</i>"+
			"<span class='remove' data-id='{{uuid}}'  data-lock='{{lockId}}'>删除</span></p>"+
	...
	thumbTemplate = "<table class='thumbTable'><tr>" +
//			"<th style='width:30px'><input type='checkbox' id='checkAll'/></th>" +
			"<th width='40px'>序号</th>" +
				"<th>材料名称</th><th class = 'oldhistory' width='50px'>历史<br/>材料数</th><th  style='width:50px'>收件<br/>未扫描</th>" +
				...
				"<td class='selectTrue'><label style='width:15px'><input type='checkbox' {{#if skipFile}}checked{{/if}}/></label></td>" + 
	...
	handlingTemplate = "<table class='handlingTable'><tr>" +
				"<th width='40px'>序号</th>" +
					"<th>事项名称</th>" +
					/*"<th>版本号</th>" +*/
					"</tr>{{#data}}{{#compare @index '0'}}" +
					"<tr class='handlingItem active {{type}}' data-id='{{uuid}}'  data-title='{{bizTitle}}' data-base-file='{{hasBaseFile}}'"+
					" data-cv='{{current_version}}' data-format='{{materialFormat}}' data-number='{{leastNumber}}'>"+
					"{{else}}<tr class='handlingItem {{type}}' data-id='{{uuid}}'   data-title='{{bizTitle}}' data-base-file='{{hasBaseFile}}'"+
					" data-cv='{{current_version}}' data-format='{{materialFormat}}' data-number='{{leastNumber}}'>{{/compare}}"+
					
					"<td><span class='handling-index'>{{@index}}</span></td>" +
					"<td style='text-align:left;{{#if hasBaseFile}}font-weight:bold{{/if}}'>{{bizTitle}}{{#if required}}<i class='star'>*</i>{{/if}}" +
					"<span class='material_time pull-right'>{{create_time}}&nbsp;</span></td>" +
					/*"<td class='version'><span class='badge-version'>" + 
					"{{yx1707}}</td>" +*/
					"</tr>{{/data}}</table>",
...
function fetchData(falg) {
	...
	if(privilege.useDYP == null || privilege.useDYP == false){
		$('.highShootDyp').hide();
	}
	
	if(privilege.useGG == null || privilege.useGG == false){
		$('.highShootGuoguang').hide();
	}
	
	if(privilege.useXH == null || privilege.useXH == false){
		$('.highShootXinhan').hide();
	}
	
	if(privilege.useFZ == null || privilege.useFZ == false){
		$('.highShootFangzheng').hide();
	}
	
	if(privilege.must == null || privilege.must == '' || privilege.must == true){
		must = true;
	}else {
		must = false;
	}
...
function updateMust(){
	for(var i in initData){
		var uuid = initData[i].uuid;
		var skipFile = initData[i].skipFile;
		var data = initData[i].children;
		pidMap[uuid] = filterCache(data) || [];
		thumbPageMap[uuid] = {
			'pageIndex' :1,
...
//// 查询时清空历史办件列表
//function clearHandlingItem(cw) {
//	//var handlingTable = cw.document.getElementsByClassName("thumbHandling")[0];
//	//handlingTable.empty();
////	var thumbHandling = cw.document.getElementById("thumbHandling");
////	$('.thumbHandling').empty();
//}
//
//// 渲染历史办件列表
//function initHandlingView(obj) {
//	if (obj == null) {
//		compileTemplate(handlingTemplate, {'data' : rootJson}, '#listView .thumbHandling');
//	} else {
//		compileSearchTemplate(handlingTemplate, {'data' : rootJson}, obj);
//	}
//		
...
function initListViewEvent(){
	...
	/*查看历史办件*/
	$('.btn-handling-history').off('click').click(function() {
		var iframe = $('#handling-history-iframe')[0];
		$('#modal-handling-history').modal({backdrop: 'static'});
		...
		var fuid = icv.fuId; // 办件id
		var subjectName = icv.config.params.subjectName; // 主体名称
		var subjectKey = icv.config.params.subjectKey;   // 主体编码
		var subjectType = icv.config.params.subjectType; // 主体类型
		...
		$('.modal-handling-ok').off('click').click(function() {
			var selectedId = cw.getSelectedId();
			var res = filextApi.copyHistoryFile({ 'oldFuid':selectedId, 'fuid':fuid });
			if (res.success) {
				showTip("引用成功！");
				updateCateView();
			}
			$('#modal-handling-history').modal('hide');
		});
...
//			//clearHandlingItem(cw);
//			
//			initHandlingView(cw.document.getElementsByClassName("thumbHandling")[0]);
//			setHandlingViewOnly();
//		});
	...
	/*紫光 高拍 by gaofx 20171108*/
	$('.highShootZiguang').off('click').click(function(){
		if ($('#uploadType').is(':checked')) {
			uploadType = 'high-shoot-ziguang';
		}
		if (isIE() && IEVersion() < 10) {
			$('#modal-upload').modal('hide');
			openHighShootZiguang();
		} else {
			$('#modal-upload').modal('hide');
			openHighShootZiguangGoogle();
		}
	});
	
	/*南天 高拍 by gaofx 20181128*/
	$('.highShootNanTian').off('click').click(function(){
		if ($('#uploadType').is(':checked')) {
			uploadType = 'high-shoot-nantian';
		}
		if (isIE()) {
			$('#modal-upload').modal('hide');
			openHighShootNanTianGoogle();
		} else {
			$('#modal-upload').modal('hide');
			openHighShootNanTianGoogle();
		}
...
//		});
		...
		$('#listView .content .collection').remove();
		$('.checkpic').remove();
		$(".operate .replace").remove();
		$(".operate .remove").remove();
		$(".operate .line").remove();
	} else {
		//dragToCategory();
		/* 本地拖拽上传 */
		...
		if (href) {
			if (href.slice(-3) == 'pdf') {
				if(isIE() && IEVersion() < 10){
					...
					icv.viewBigPdf_Pdfjs(href);
					var pdfHeight = doc.height() - 6;//iframe border=2
					pdfJS.show(href, 'viewBigPdf');
					doc.find('.view-big-container .media').css('margin-top', '50px');
				}
			} else {
				showTip('此文件暂不支持预览！');
	...
	/* 下载图片 */
	$('.operate .download').off('click').click(function() {
		var id = $(this).data('id');
		var obj = getObject(id);
		window.open(obj.file + '&d=true');
	});
	...
	/* 打印图片 */
	$('.operate .print').off('click').click(function() {
		var id = $(this).data('id');
		var obj = getObject(id);
//		window.open("printPic.html?picSrc="+obj.file);
		var picLabel = '<img src="' + obj.file + '" />';
		 LODOP.PRINT_INITA(0,0,842,595,"测试C-Lodop远程打印四步骤");
		 LODOP.ADD_PRINT_IMAGE(0, 0, "100%", "100%", picLabel);
	...
	$('.selectTrue input').off('click').click(function(){
		var dataId = $(this).parent().parent().parent().data('id');
		var c = $(this);
		var currentRequired = idMap[dataId].required;
		var check = $(c).is(':checked');
		if (check) {
			var result = filextApi.collectNotScan({
				uuid:dataId,
				skip:check,
			});
			if(result){
				idMap[dataId].skipFile = true;
			}
        }else {
        	var result = filextApi.collectNotScan({
				uuid:dataId,
...
//提供办件查询时的渲染接口
function compileSearchTemplate(source, data, container) {
	var template = Handlebars.compile(source);
	Handlebars.registerHelper("compare", function(v1, v2, options) {
		if (v1 == v2) {
			return options.fn(this);
...
function uploadModalShow(){
	...
	}else if (uploadType == 'high-shoot-ziguang'){//紫光高拍 by yuanzp
		if (isIE() && IEVersion() < 10) {
			openHighShootZiguang(); return ;
		} else {
			showTip('高拍功能仅支持IE浏览器！');
		}
	}else if (uploadType == 'high-shoot-guoguang'){//紫光高拍 by yuanzp
		if (isIE() && IEVersion() < 10) {
			openHighShootGuoguang(); return ;
		} else {
			showTip('高拍功能仅支持IE浏览器！');
		}
	}else if(uploadType == 'high-shoot-nantian'){
		if(!isIE()){
			openHighShootNanTianGoogle(); return ;
		}else {
			showTip('南天高拍功能不支持IE浏览器！');
		}
...
/* 打开良田高拍Google界面   by gaofx 20180419*/
function openHighShootGoogle() {
	window.showModalDialog('high-shoot-lt-google.html', window, "dialogHeight:600px;dialogWidth:1000px");
}
...
/* 打开紫光高拍界面   by gaofx 20171108*/
function openHighShootZiguang() {
	window.showModalDialog('high-shoot-weishan.html', window, "dialogHeight:600px;dialogWidth:1000px;");
}

/* 打开紫光高拍谷歌界面   by gaofx 20171108*/
function openHighShootZiguangGoogle() {
	window.showModalDialog('high-shoot-ws-google.html', window, "dialogHeight:600px;dialogWidth:1000px");
}
...
/* 打开南天高拍界面   by gaofx 20181108*/
function openHighShootNanTianGoogle(){
	window.showModalDialog('high-shoot-nantian.html', window, "dialogHeight:600px;dialogWidth:1000px");
}
