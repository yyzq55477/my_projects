...
function FilextApi(conf) {
	...
	this.commonCall = function(setting) {
		...
		if(ajaxData.indexOf("initFileUnit") != -1 
				|| ajaxData.indexOf("queryAllPic") != -1
				|| _timestamp == -1) {
			var params = "method=initTimestamp";
			var initData = null;
			if(this.config.config.params!=null && this.config.config.params.initData!=null) {
				initData = this.config.config.params.initData;
				$.each(initData, function (k, v) {
					params += ("&"+k+"="+v);
				});
			}
			initData = this.encode64(params);
			$.ajax({
...
FilextApi.prototype.getInitData = function(falg) {
	...
	var res = this.commonCall({
		data : {
			...
			isfw : me.config.config.params.isfw,
			iswb : me.config.config.params.iswb,
...
FilextApi.prototype.doUploadNewMethod = function(config, callback) {
	...
	var uploader = WebUploader
			.create({
				...
				// 只允许选择图片文件。
				accept : {
					extensions : fileTypes||'gif,jpg,jpeg,bmp,png,tif,doc,docx,xls,xlsx,ppt,pptx,txt,pdf,mp3,wav,swf,avi,mp4,rar,zip,json'
				},
	...
	var thumbnailHeight = 30;
		var uploader = WebUploader
			.create({
				...
				// 只允许选择图片文件。
				accept : {
					extensions : 'gif,jpg,jpeg,bmp,png,tif,doc,docx,xls,xlsx,ppt,pptx,txt,pdf,mp3,wav,swf,avi,mp4,rar,zip,json'
				},
				fileNumLimit : 4,
				fileSizeLimit : 500*1024*1024,
				method : 'POST',
...
//查看历史办件
FilextApi.prototype.viewHandlingHistory = function(config) {
	var res = this.commonCall({
		data : {
			method : 'viewHandlingHistory',
			uuid : config.uuid,
			subjectName : config.subjectName,
			subjectKey : config.subjectKey,
			subjectType: config.subjectType,
...
//根据办件ID查看办件材料
FilextApi.prototype.copyHistoryFile = function(config) {
	var res = this.commonCall({
		data : {
			method : 'copyHistoryFile',
			fuid : config.fuid,
			oldFuid : config.oldFuid
		}
	});
...
//根据办件ID查看办件材料
FilextApi.prototype.viewMaterial = function(config) {
	var res = this.commonCall({
		data : {
			method : 'viewMaterial',
			fuid : config.fuid,
...
//收取未扫描控制，调过验证
FilextApi.prototype.collectNotScan = function(config) {
	var res = this.commonCall({
		data : {
			method : 'collectNotScan',
			uuid : config.uuid,
			skip : config.skip
		}
	});
