﻿﻿var filextApi = null,
	...
	template = "<input id='pic-search' type='text' class='form-control' placeholder='搜索图片名称' /><ul id='items' class='clearfix'>"+
			...
			"{{#data}}<li class='item {{type}}Cls' id='item_{{uuid}}' data-id='{{uuid}}' data-lock='{{lockId}}'" +
			"data-href='{{var-0}}' title='{{title}}'>"+
	...
	thumbTemplate = "<ul>{{#data}}{{#compare @index '0'}}"+
				...
				" data-cv='{{current_version}}' data-format='{{materialFormat}}' " +
				"data-number='{{leastNumber}}' data-lock='{{lockId}}'>"+
				"{{else}}<li class='thumbItem clearfix {{type}}' data-id='{{uuid}}' data-title='{{bizTitle}}' data-base-file='{{hasBaseFile}}'"+
				" data-cv='{{current_version}}' data-format='{{materialFormat}}' data-number='{{leastNumber}}'  data-lock='{{lockId}}'>{{/compare}}"+
...
function updateCurrentData(id) {
	...
	var lockId = currentLockId;
	var res = filextApi.getSubData(updateId,lockId);
	var data = res.data || [];
	if (res.success) {
...
function initListView(){
	...
	if($('#listView .thumb li.active').length > 0)
		...
		currentLockId = getCurrentLockId();
	updateContent();
	$('#listView').show();
	$('#editView').hide();
	$('#detailView').hide();
...
function initListViewEvent(){
	...
	$('.btn-pic-save').off('click').click(function(){
		var msg=filextApi.updateTemporary(filextApi.uuid,filextApi.lockId);
		if(msg.success=true){
			alert("保存成功");
		}else{
			alert("保存失败");
	...
	$('.badge-version').off('click').click(function(e) {
		...
		var lockId_ = parentEl.data('lock');
		cv = parentEl.data('cv'),
		...
		$('.modal-history-ok').off('click').click(function() {
			...
			var res = filextApi.updateCurrentMaterial({ 'src':selectedId, 'dest':id,'lockId':lockId_ });
			if (res.success) {
				showTip("引用成功！");
				updateCateView();
			}
...
function printImages(images) {
	...
	$('#detailView .content img').each(function() {
		var id = $(this).parent().parent().data('id');
		var lockId_ = $(this).parent().parent().data('lock');
...
function getCurrentLockId() {
	return $('#listView .thumb li.active').data('lock');
}
