...
public class ProxyServlet extends HttpServlet {
	...
	@SuppressWarnings({ "deprecation", "rawtypes", "unchecked" })//消除对弃用方法的警告提示
	void proxy(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if("createAndPutStorageObject".equals(params.get("method")) || "1".equals(params.get("isHighShoot"))) {
			// 默认高拍仪操作是安全的
			...
			try {
				...
				String urlParam = new String(base64.decodeBase64(params.get("data")), StandardCharsets.UTF_8);
		...
		params.put("isProxy", "true"); // 是否经由前台页面发起请求 
		
		if("initTimestamp".equals(params.get("method"))) {
			try {
				ServletConfig config = getServletConfig();
				String validatorPath = config.getInitParameter("VALIDATE_PATH");
				if(validatorPath != null){
					String classPath = validatorPath.substring(0, validatorPath.lastIndexOf("."));
					String methodName = validatorPath.substring(validatorPath.lastIndexOf(".")+1, validatorPath.length());
					Class clazz = Class.forName(classPath);
					Method method = clazz.getDeclaredMethod(methodName, Map.class);
					Object object = method.invoke(clazz.newInstance(), params);
					if(!(Boolean)object) {
						System.out.println("业务系统校验不通过...");
						return;
					}
				}
			} catch (Exception e) {
		...
		// 如果方法不是initFileUnit,获取其时间戳判断时效性 
		// 服务器时间可能与本地时间不一致，取消本地时间戳校验
		/*if(!"initTimestamp".equals(params.get("method"))) {
			long webTime = Long.valueOf(params.get("timestamp")); // 前台时间戳
			long javaTime = System.currentTimeMillis() / 1000;    // 后台时间戳
			// 如果时间戳相差20秒，则判断时间戳无效，需重新初始化
			if(webTime < (javaTime-20)) {
				outDetailRequest(req, params);
				System.out.println("请求时间超时，请重新初始化");
				return;
			}
		}*/
	...
	public String execRemoteMethod(String nodeName, String method, HttpMethod httpMethod, Map<String, String> params,
		...
		if (httpMethod == HttpMethod.GET) {
			urlString = SpUtils.combine(getNodeEndpoint(nodeName) + "?method=" + method, params);
			params.clear();
		} else if (httpMethod == HttpMethod.POST && !(method.equals("putStorageContent") ||method.equals("createAndPutStorageObject"))) {
			urlString = getNodeEndpoint(nodeName);
		...
		SpUtils.log("...method-> " + method);
		
		/*for (Map.Entry<String,String> entry : params.entrySet()) { 
		  System.out.println("Key = " + entry.getKey() + ", Value = " + entry.getValue()); 
		}*/
		
		HttpResultEntity result = null;
		if (method.equals("putStorageContent")||method.equals("createAndPutStorageObject")) {
			HttpPost httpRequest = new HttpPost(urlString);
			copyRequest(request,httpRequest,isScanOnWeb);
			result = HttpClientPoolUtil.httpClientRequestGet(httpRequest,null);
			...
			HttpPost httpRequest = new HttpPost(urlString);
			copyRequest(request,httpRequest,isScanOnWeb);
			result = HttpClientPoolUtil.httpClientRequestPost(httpRequest, params, null);
		}
		
		String res = result.getBody();
		SpUtils.log("...result-> " + res);
		return res;
	 ...
	 * 设置请求参数
	 */
	private void copyRequest (final HttpServletRequest request,HttpRequestBase httpRequest,boolean isScanOnWeb) {
		if (request != null) {
...
//		HttpClientProperties.APIROUTE = (getInitParameter(SERVER_URL_NAME)+ CONTEXT_PATH + "/service/filext-api");
		apiroute = (getInitParameter(SERVER_URL_NAME)+ CONTEXT_PATH + "/service/filext-api");
